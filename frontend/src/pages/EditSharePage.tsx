...
import { Plus, Code, Image, X, FileText, ArrowUp, ArrowDown, Save, XCircle } from 'lucide-react';
...
const EditSharePage: React.FC = () => {
  ...
  const loadShare = async () => {
    try {
      ...
      const share = await shareApi.getShare(id!);
      ...
      // Set code files
      if (share.codeSnippets && share.codeSnippets.length > 0) {
        const files: CodeFile[] = share.codeSnippets.map((snippet, index) => ({
          id: (index + 1).toString(),
          name: snippet.filename || '',
          language: snippet.language || 'kotlin',
      ...
      // Set image URLs
      if (share.imageUrls && share.imageUrls.length > 0) {
        setImageUrls(share.imageUrls);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await shareApi.updateShare(id!, shareData);
      navigate('/profile');
    } catch (error: any) {
      console.error('Failed to update share:', error);
      let errorMessage = 'Failed to update share. Please try again.';
      
      if (error.response?.status === 400) {
        errorMessage = error.response.data || 'Invalid data. Please check your input.';
      } else if (error.response?.status === 401) {
        errorMessage = 'You must be logged in to update shares.';
      } else if (error.response?.status === 403) {
        errorMessage = 'You do not have permission to update this share.';
  ...
  // Helper functions for managing code files (same as CreateSharePage)
  const addCodeFile = () => {
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-16">
            ...
            <div className="ml-6">
              <h1 className="text-xl font-semibold text-gray-700">Edit Share</h1>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center mb-6">
            ...
            <h1 className="text-2xl font-bold text-gray-900">Edit Share</h1>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end gap-4">
              <button
                ...
                className="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 focus:ring-2 focus:ring-gray-500 flex items-center"
              >
